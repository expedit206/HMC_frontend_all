<template>
  ...
  >
    <!-- Image -->
    <div
      class="relative aspect-[4/3] overflow-hidden rounded-t-2xl bg-gray-100 flex items-center justify-center"
    >
      <div
        v-if="
          product.image &&
      ...
      <div class="absolute top-3 left-3 flex gap-2">
        <span
          v-if="product.isNew"
          class="px-2.5 py-1 text-xs font-bold text-white bg-secondary rounded-full shadow-sm"
        >
          Nouveau
        </span>
        <span
      ...
      >
        ...
        <button
          @click.stop="emit('toggleClientFavorite', product)"
    ...
    <div class="p-4">
      ...
      <div class="flex justify-between items-start mb-2">
        <span
          class="text-xs font-medium text-primary uppercase tracking-wider bg-primary/10 px-2 py-1 rounded"
        >
          {{ product.category }}
        </span>
      ...
      <!-- Title -->
      <h3
        class="font-bold text-gray-800 text-lg mb-1 line-clamp-1 group-hover:text-primary transition-colors"
      >
        {{ product.name }}
      </h3>
      ...
      <!-- Price & Button -->
      <div
        class="flex items-center justify-between pt-3 border-t border-border mt-auto"
      >
        <div class="flex flex-col">
          <span
            v-if="product.oldPrice"
            class="text-xs text-gray-400 line-through decoration-red-500"
          >
