<template>
  <div v-if="sidebarStore.isMobileOpen" @click="sidebarStore.toggleMobile()"
    class="md:hidden fixed inset-0 top-[64px] sm:top-[80px] bg-black/50 backdrop-blur-sm z-40 transition-opacity"></div>
  <aside :class="[
    ...
    <div class="px-3 mb-6 relative">
      <template v-if="authStore.isAuthenticated">
        <div class="flex items-center gap-3 p-2 rounded-xl hover:bg-muted/50 transition-colors group">
          <RouterLink to="/parametres" class="flex items-center gap-3 flex-1 min-w-0">
            <div
              class="w-12 h-12 rounded-full border border-border bg-muted overflow-hidden flex-shrink-0 shadow-sm group-hover:border-primary transition-colors">
              <img v-if="userAvatar" :src="userAvatar" alt="Avatar" class="w-full h-full object-cover">
              <div v-else
                class="w-full h-full flex items-center justify-center bg-primary text-white font-bold text-sm">
                {{ userInitials }}
              </div>
            </div>
      ...
      <template v-else>
        <div class="p-4 rounded-2xl bg-gradient-to-br from-primary/10 to-secondary/10 border border-primary/20">
          ...
          <p class="text-[11px] text-muted-foreground mb-4">Connectez-vous pour accéder à toutes les fonctionnalités.
          </p>
          <RouterLink to="/auth/connexion"
            class="w-full py-2 px-4 bg-primary text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 hover:bg-primary/90 transition-all shadow-lg shadow-primary/20">
      ...
      <Transition name="fade-slide">
        <div v-if="showRoleSwitcher"
          class="absolute top-[calc(100%+4px)] left-3 right-3 bg-card rounded-xl shadow-xl border border-border p-2 z-50 overflow-hidden">
          <p class="text-[9px] font-black text-muted-foreground uppercase tracking-widest pl-2 mb-2">Changer d'espace
          </p>
          <button v-for="r in allRoles" :key="r" @click="handleSwitchRole(r)"
            ...
            <div class="w-6 h-6 rounded-md flex items-center justify-center"
              :class="r === activeRole ? 'bg-white/20' : 'bg-muted'">
    ...
    <nav class="flex-1 px-3 space-y-1">
      ...
      <template v-for="(link, index) in filteredLinks" :key="index">
        <!-- Label de Section -->
        <p v-if="link.isLabel"
          class="px-4 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] mt-6 mb-2">
          {{ link.name }}
        </p>

        <!-- Lien Normal -->
        <RouterLink v-else :to="link.to"
          class="flex items-center gap-3 px-3 py-2.5 rounded-lg font-semibold transition-colors nav-link"
          active-class="active">
          <div class="w-8 flex justify-center relative">
            <i :class="[link.icon, 'text-lg']"></i>
            <span v-if="link.badge"
              class="absolute -top-1 -right-1 bg-secondary text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full border border-card min-w-[18px] text-center">
              {{ link.badge }}
            </span>
          </div>
          <span>{{ link.name }}</span>
    ...
    <div class="mt-auto px-3 border-t border-border pt-4 pb-2">
      ...
      <button @click="logout"
        class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg font-semibold transition-colors text-red-500 hover:bg-red-500/10">
