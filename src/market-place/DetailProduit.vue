  ...
  <div class="bg-background min-h-screen pb-20">
    ...
    <div v-else-if="item" class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
      ...
      <nav class="flex items-center gap-2 mb-8 text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
        <router-link :to="{ name: 'MarketplaceIndex' }" class="hover:text-primary transition-colors flex items-center gap-1.5">
      ...
      <div class="bg-card rounded-2xl shadow-xl shadow-black/5 border border-border overflow-hidden">
        <div class="flex flex-col lg:flex-row">
          ...
          <div class="lg:w-1/2 p-6 lg:p-10 border-b lg:border-b-0 lg:border-r border-border/50 bg-muted/10 relative">
            <div class="aspect-squarerounded-xl overflow-hidden bg-muted/30 flex items-center justify-center relative group">
              ...
              <!-- Tag "Nouveau" -->
              <div v-if="item.isNew" class="absolute top-6 left-6 px-3 py-1 bg-secondary text-secondary-foreground text-[10px] font-black uppercase tracking-widest rounded shadow-sm">
                Nouveau
              </div>
            </div>
            ...
            <div class="grid grid-cols-4 gap-3 mt-4">
              <div class="aspect-square rounded-lg bg-card border-2 border-primary cursor-pointer overflow-hidden opacity-100 p-2">
                 <img v-if="item.image && !item.image.startsWith('fas ')" :src="item.image" class="w-full h-full object-contain mix-blend-multiply" @error="onImageError"/>
                 <i v-else :class="item.image || 'fas fa-box'" class="w-full h-full flex items-center justify-center text-primary/30 text-2xl"></i>
              </div>
              <div v-for="i in 3" :key="i" class="aspect-square rounded-lg bg-muted/30 border-2 border-transparent hover:border-primary/30 cursor-pointer transition-all overflow-hidden opacity-60 hover:opacity-100 flex items-center justify-center">
          ...
          <div class="lg:w-1/2 p-6 lg:p-10 flex flex-col">
            ...
            <div class="mb-6">
              <div class="flex items-center justify-between mb-3">
                <span class="inline-flex px-2.5 py-1 bg-primary/10 text-primary text-[10px] font-black uppercase tracking-widest rounded">
                  {{ item.category }}
                </span>
                <div class="flex items-center gap-1.5 text-xs text-yellow-500 font-bold bg-yellow-500/10 px-2 py-1 rounded">
              ...
              <div class="flex items-center gap-2 text-[11px] font-bold text-muted-foreground uppercase tracking-widest">
            ...
            <div class="mb-8 p-6 bg-muted/20 rounded-xl border border-border/50 relative overflow-hidden">
              <div class="absolute right-0 top-0 w-32 h-32 bg-primary/5 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2"></div>
              ...
              <div class="relative z-10">
                ...
                <div class="text-3xl font-black text-primary flex items-baseline gap-2">
                  ...
                  <span v-if="item.discount" class="text-xs font-bold text-white bg-red-500 px-2 py-0.5 rounded-md align-middle shadow-sm">
                    -{{ item.discount }}%
                  </span>
                </div>
                
                <div class="mt-4 flex items-center gap-2 text-xs font-bold" :class="item.stock === 0 ? 'text-red-500' : 'text-green-600'">
            ...
            <div class="mb-8 flex-1">
              <h3 class="text-[10px] font-black text-foreground uppercase tracking-[0.2em] mb-3">À propos de cet article</h3>
              <p class="text-sm text-muted-foreground leading-relaxed whitespace-pre-line">
                {{ item.description || "Aucune description détaillée n'a été fournie pour cet article. Veuillez contacter le vendeur pour plus d'informations." }}
              </p>
            </div>
            ...
            <div class="mt-auto space-y-3 pt-6 border-t border-border">
              ...
              <div class="flex gap-3 h-12">
                <div class="flex items-center border border-border bg-card rounded-lg overflow-hidden w-28 shrink-0">
                  <button @click="quantity = Math.max(1, quantity - 1)" class="w-10 h-full text-muted-foreground hover:text-foreground hover:bg-muted transition-colors flex items-center justify-center">
                    <i class="fas fa-minus text-xs"></i>
                  </button>
                  <input type="number" v-model="quantity" class="flex-1 w-full text-center text-sm font-black border-none focus:ring-0 p-0" min="1" />
                  <button @click="quantity++" class="w-10 h-full text-muted-foreground hover:text-foreground hover:bg-muted transition-colors flex items-center justify-center">
        ...
        <div class="bg-muted/30 border-t border-border/50 p-6 lg:px-10 flex flex-col sm:flex-row items-center gap-5 justify-between">
          <div class="flex items-center gap-4">
            <div class="w-14 h-14 rounded-full bg-card border border-border shadow-sm flex items-center justify-center overflow-hidden shrink-0 text-primary">
              <img v-if="item.seller?.avatar" :src="item.seller.avatar" class="w-full h-full object-cover" @error="e => e.target.style.display='none'" />
              <i v-else class="fas fa-store text-xl"></i>
            </div>
            <div>
              <p class="text-[9px] font-black text-muted-foreground uppercase tracking-widest mb-1">Vendu et expédié par</p>
          ...
          <button class="w-full sm:w-auto px-6 py-2.5 bg-card border border-border rounded-md text-xs font-bold text-foreground hover:bg-muted transition-colors">
            Voir la boutique
          </button>
        </div>
