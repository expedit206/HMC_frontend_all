      ...
      <div class="max-w-3xl mx-auto">
        ...
        <div class="mb-10 flex justify-between items-end">
          <div>
            ...
            <p class="text-gray-500 font-bold uppercase tracking-[0.2em] text-[11px] mt-4 italic">
              Étape {{ currentStep }} sur 3 : {{ stepTitles[currentStep - 1] }}
            </p>
          </div>
          <RouterLink :to="{ name: 'BailleurDashboard' }"
        ...
        <div class="mb-8 bg-blue-50 border-l-4 border-blue-500 p-4 rounded-xl">
          <div class="flex items-start gap-3">
            ...
            <p class="text-xs text-blue-700 font-bold uppercase tracking-wide leading-relaxed">
        ...
        <div class="mb-10 flex gap-2">
          <div v-for="step in 3" :key="step" class="h-1.5 flex-1 rounded-full transition-all duration-500"
            :class="step <= currentStep ? 'bg-[#E54801]' : 'bg-gray-200'"></div>
        </div>
        ...
        <form @submit.prevent="submitForm" class="space-y-8 animate-fadeIn">
          ...
          <section v-if="currentStep === 1"
            ...
            <div class="flex items-center gap-3 mb-6">
              ...
              <h3 class="text-xs font-black text-[#1B0B38] uppercase tracking-widest">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="col-span-1 md:col-span-2">
                <label class="block text-[10px] text-gray-400 font-bold uppercase mb-2 ml-2">Titre du bien</label>
                <input v-model="form.title" type="text" required placeholder="Ex: Villa de luxe à Bastos"
              ...
              <div>
                <label class="block text-[10px] text-gray-400 font-bold uppercase mb-2 ml-2">Type d'offre</label>
              ...
              <div>
                <label class="block text-[10px] text-gray-400 font-bold uppercase mb-2 ml-2">Catégorie</label>
              ...
              <div>
                <label class="block text-[10px] text-gray-400 font-bold uppercase mb-2 ml-2">Prix estimé (XAF)</label>
                <input v-model.number="form.price_estimate" type="number" required placeholder="0"
          ...
          <section v-if="currentStep === 2"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input v-model="form.city" type="text" required placeholder="Ex: Yaoundé"
              ...
              <div>
                <label class="block text-[10px] text-gray-400 font-bold uppercase mb-2 ml-2">Quartier / Adresse
                  précise</label>
                <input v-model="form.location" type="text" required placeholder="Ex: Bastos, Rue de l'Ambassade"
                  class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-[#E54801]/20 outline-none transition-all text-gray-700" />
              </div>

              <div class="col-span-1 md:col-span-2">
                <label class="block text-[10px] text-gray-400 font-bold uppercase mb-2 ml-2">Description
                  sommaire</label>
          ...
          <section v-if="currentStep === 3"
            ...
            <div class="mb-8">
              <label class="block text-[10px] text-gray-400 font-bold uppercase mb-4">Documents (CNI, Titre foncier,
                Mandat...)</label>
              <div @click="$refs.fileInput.click()"
                ...
                <input ref="fileInput" type="file" multiple accept="application/pdf,image/*" class="hidden"
                ...
                <div class="relative z-10">
                  <div
                    ...
                    <i class="fas fa-file-contract text-xl text-gray-300 group-hover:text-[#E54801]"></i>
                  </div>
                  <p class="text-[11px] font-bold text-gray-500 group-hover:text-[#1B0B38]">
              ...
              <!-- Previews -->
              <div v-if="previews.length" class="grid grid-cols-4 sm:grid-cols-6 gap-3 mt-4">
                <div v-for="(src, idx) in previews" :key="idx" class="relative aspect-square group">
                  <div v-if="src.isPdf"
                    ...
                    <p class="absolute bottom-1 w-full text-center text-[8px] font-black text-red-500 truncate px-1">
                      {{ src.name }}
                    </p>
                  </div>
                  <img v-else :src="src.url"
...
const requestId = ref(route.query.edit || null);
...
onMounted(async () => {
  if (isEditMode.value) {
    try {
      ...
      if (data.success) {
        const req = data.data.find(r => r.id == requestId.value);
        if (req) {
          Object.assign(form, {
            title: req.title,
            type: req.type,
          ...
          // Note: On ne peut pas facilement pré-remplir les fichiers previews sans les télécharger, 
          // mais l'utilisateur verra les fichiers existants dans le détail.
        }
      }
    } catch (err) {
...
const submitForm = async () => {
  ...
  if (!isEditMode.value && form.documents.length === 0) {
    return alert(
  ...
  try {
    ...
    if (isEditMode.value) {
      // Pour l'édition, Laravel peut avoir des soucis avec PUT + FormData
      // On peut simuler un PUT avec un POST et un champ _method
      formData.append("_method", "PUT");
      const { data } = await axios.post(
        `/api/bailleur/publication-requests/${requestId.value}`,
        formData,
        {
      ...
      if (data.success) {
        alert("Votre demande de publication a été mise à jour avec succès.");
        router.push({ name: "PublicationRequests" });
      }
    } else {
      const { data } = await axios.post(
