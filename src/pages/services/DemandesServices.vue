  ...
  <div class="min-h-screen bg-background">
    ...
    <header class="bg-card border-b border-border sticky top-0 z-30">
      <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <div class="flex items-center gap-3">
          ...
          <div class="hidden md:flex items-center gap-2">
            ...
            <div class="relative">
              ...
              <select
                ...
                @change="fetchJobs()"
                class="pl-8 pr-8 py-2.5 bg-muted/50 border border-border rounded-xl text-xs font-bold text-foreground focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition-all appearance-none cursor-pointer min-w-[140px]"
              >
          ...
          <!-- Recherche SUPPRIMÉE car gérée par le Header -->
          ...
          <div v-if="mobileMenuOpen" class="md:hidden mt-3 pb-3 border-t border-border pt-3 space-y-3">
            ...
            <div>
              ...
              <select
                ...
                class="w-full px-4 py-2.5 bg-muted/50 border border-border rounded-xl text-sm font-bold text-foreground focus:ring-2 focus:ring-primary/20 transition-all"
              >
                <option v-for="cat in categories" :key="cat.id" :value="cat.id">{{ cat.name }}</option>
              </select>
            </div>
            ...
            <div>
              ...
              <select
                v-model="activeCity"
                @change="onCityChange"
                class="w-full px-4 py-2.5 bg-muted/50 border border-border rounded-xl text-sm font-bold text-foreground focus:ring-2 focus:ring-secondary/20 transition-all"
              >
                <option value="Tout le pays">Tout le pays</option>
                <option v-for="city in cities" :key="city" :value="city">{{ city }}</option>
              </select>
    ...
    <main class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <h2 class="text-xl font-black text-foreground">Missions Disponibles</h2>
      ...
      <div v-if="isLoading && jobs.length === 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="n in 6" :key="n" class="h-48 bg-card rounded-3xl border border-border animate-pulse shadow-sm"></div>
      </div>
      ...
      <div v-else-if="!isLoading" class="flex flex-col items-center justify-center py-32 text-center">
        <div class="w-20 h-20 bg-muted rounded-full flex items-center justify-center mb-6">
          <i class="fas fa-clipboard-check text-3xl text-muted-foreground/50"></i>
        </div>
        <h3 class="text-lg font-bold text-foreground">Aucune mission trouvée</h3>
        <p class="text-muted-foreground mt-2 text-sm max-w-xs mx-auto">Il n'y a pas de demande correspondant à vos critères pour le moment.</p>
      ...
      <div ref="scrollTarget" class="py-12 flex justify-center">
        <div v-if="isLoading && jobs.length > 0" class="flex items-center gap-3">
          ...
          <span class="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Recherche de missions...</span>
        </div>
        <div v-else-if="!hasMorePages && jobs.length > 0" class="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/50 italic">
