  ...
  <div class="dashboard-page-content">
    <div class="max-w-6xl mx-auto">
      ...
      <div class="space-y-8">
        ...
        <template v-else-if="rentals.length > 0">
          <div v-for="rental in rentals" :key="rental.id"
            ...
            <div class="md:w-72 h-56 md:h-auto overflow-hidden relative">
              <img :src="rental.property?.primary_image?.path ||
                'https://images.unsplash.com/photo-1512917774080-9991f1c4c750?auto=format&fit=crop&w=800&q=80'
                " class="w-full h-full object-cover" :class="{ 'grayscale opacity-70': rental.status !== 'active' }" />
              <div v-if="rental.status !== 'active'"
                class="absolute inset-0 bg-black/20 flex items-center justify-center">
                <span
                  class="bg-black/60 text-white text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-full border border-white/20">Terminée</span>
              </div>
            </div>
            <div class="flex-1 p-6 md:p-8">
              <div class="flex flex-col md:flex-row justify-between gap-4 mb-6">
                <div>
                  <div class="flex items-center gap-2 mb-2">
                    <span v-if="rental.status === 'active'"
                      class="px-2.5 py-1 bg-green-100 dark:bg-green-950/30 text-green-600 dark:text-green-400 text-[10px] font-bold rounded-lg uppercase tracking-wide">En
                      cours</span>
                    <span class="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">Contrat #LOC-{{
                      String(rental.id).padStart(4, "0")
                    }}</span>
                  </div>
                  <h2 class="text-xl font-bold" :class="rental.status === 'active'
                      ? 'text-foreground'
                      : 'text-muted-foreground'
                    ">
                    {{ rental.property?.title }}
                  </h2>
                  <p class="text-xs mt-1" :class="rental.status === 'active'
                      ? 'text-muted-foreground'
                ...
                <div class="text-left md:text-right">
                  <p class="text-xl font-black" :class="rental.status === 'active'
                      ? 'text-secondary'
                  ...
                  <p class="text-[10px] font-bold uppercase mt-1" :class="rental.status === 'active'
                      ? 'text-muted-foreground'
                      : 'text-muted-foreground/50'
                    ">
                    Loyer HT
              ...
              <div class="grid grid-cols-2 lg:grid-cols-4 gap-6 p-5 bg-muted/20 rounded-2xl mb-8">
                <div>
                  <p class="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mb-1 leading-none">
                    Début
                  </p>
                  <p class="text-xs font-bold" :class="rental.status === 'active'
                      ? 'text-foreground'
                      : 'text-muted-foreground'
                    ">
              ...
              <div class="flex flex-wrap gap-3" v-if="rental.status === 'active'">
                <RouterLink v-if="rental.status === 'active'" to="/locataire/detail-mon-bien"
        ...
        <!-- Empty State -->
        <div v-else class="bg-muted/10 border-2 border-dashed border-border rounded-3xl p-16 text-center">
          <div
          ...
          <h3 class="text-xl font-bold text-muted-foreground uppercase tracking-tight">
