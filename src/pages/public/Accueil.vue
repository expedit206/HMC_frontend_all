  ...
  <div class="accueil-page bg-background">
    ...
    <section class="hero-root relative overflow-hidden">
      ...
      <transition-group name="hero-bg" tag="div" class="absolute inset-0">
        <div v-for="(slide, i) in heroSlides" v-show="currentSlide === i" :key="slide.role"
          class="absolute inset-0 hero-bg-item" :style="{ backgroundImage: `url(${slide.bg})` }"></div>
      </transition-group>
      ...
      <div class="relative z-10  px-4 sm:px-6 lg:px-8 pt-8 pb-6 lg:pt-12 lg:pb-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-8 items-center">
          ...
          <div class="lg:col-span-3">
            ...
            <!-- ── Search CTA ── -->
            <transition name="slide-up-delay2" mode="out-in">
              <div :key="'cta-' + currentSlide" class="mt-8 flex flex-wrap gap-4">
                <button @click="isSearchOpen = true"
                  class="px-8 py-4 bg-primary text-primary-foreground font-bold rounded-2xl shadow-xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all flex items-center gap-3">
                  <i class="fas fa-search"></i>
                  Commencer une recherche
                </button>
                <RouterLink :to="heroSlides[currentSlide].ctaPrimary.link"
                  class="px-8 py-4 bg-white/10 backdrop-blur-md text-white font-bold rounded-2xl border border-white/20 hover:bg-white/20 transition-all flex items-center gap-3">
                  <i :class="'fas fa-' + heroSlides[currentSlide].ctaPrimary.icon"></i>
            ...
            <transition name="slide-up-delay3" mode="out-in">
              <div :key="'stats-' + currentSlide" class="flex flex-wrap gap-3">
                <div v-for="stat in heroSlides[currentSlide].stats" :key="stat.label"
          ...
          <div class="hidden lg:block lg:col-span-2 relative h-full min-h-[400px]">
            <!-- On pourrait mettre ici une illustration 3D ou un élément immersif -->
            <div class="absolute inset-0 flex items-center justify-center">
              <div
                class="relative w-full aspect-square max-w-sm rounded-[3rem] overflow-hidden border-8 border-white/10 shadow-2xl skew-y-3 hover:skew-y-0 transition-transform duration-500">
                <img :src="heroSlides[currentSlide].bg" class="w-full h-full object-cover scale-110">
        ...
        <div class="role-nav-bar mt-8">
          <button v-for="(slide, i) in heroSlides" :key="slide.role" @click="goToSlide(i)"
            ...
            <span class="role-nav-icon" :style="currentSlide === i ? { background: slide.accentBg } : {}">
              <i :class="'fas fa-' + slide.icon" :style="currentSlide === i ? { color: slide.accentText } : {}"></i>
            </span>
            ...
            <span v-if="currentSlide === i" class="role-nav-progress"
              :style="{ background: slide.accentText, animationDuration: slideDuration + 'ms' }"></span>
          </button>
        </div>
      </div>
    </section>
          ...
          <div v-for="property in newProperties" :key="property.id"
            ...
            <div class="h-36 relative overflow-hidden group/image">
              ...
              <div class="absolute top-2 left-2 pointer-events-none">
                <span
                  class="bg-secondary text-secondary-foreground text-[10px] font-semibold px-2 py-0.5 rounded-full">Nouveau</span>
              </div>
              ...
              <template v-if="property.all_images && property.all_images.length > 1">
                ...
                <div class="absolute bottom-1 left-1/2 -translate-x-1/2 flex gap-1 z-30 pointer-events-none">
                  <div v-for="(_, idx) in property.all_images" :key="idx" class="w-1 h-1 rounded-full transition-all"
              ...
              <PropertyActionBar :property="property" :is-fav="favoritesAccueil.has(property.id)" btn-size="sm"
                position="absolute right-2 bottom-1.5" @toggle-favorite="toggleFavoriteAccueil"
...
const cities = ["Douala", "Yaoundé", "Bafoussam", "Bamenda", "Garoua", "Maroua", "Ngaoundéré", "Bertoua", "Limbe",
"Kribi"];
const propertyTypes = ["Studio", "Appartement", "Chambre moderne", "Villa", "Duplex", "Immeuble", "Terrain", "Bureau",
"Local commercial"];
...
if (searchFilters.value.budget && searchFilters.value.budget < 500000) { query.max_price=searchFilters.value.budget; }
  router.push({ name: 'Annonces' , query }); }; const fetchData=async ()=> {
...
══════════════════════════════════════════════════════ */
  ...
  .hpt-5 {
    ...
    animation-delay: 8s;
    background: hsl(var(--chart-5) / 0.5);
  }
  ...
  .hpt-9 {
    ...
    animation-delay: 4s;
    background: hsl(var(--chart-4) / 0.6);
  }
  ...
  .hpt-11 {
    ...
    background: hsl(var(--chart-5) / 0.4);
  }
  ...
  .hpt-13 {
    ...
    animation-duration: 20s;
    animation-delay: 11s;
    background: hsl(var(--chart-1) / 0.6);
  }
  ...
  .hpt-15 {
    ...
    animation-duration: 17s;
    animation-delay: 10s;
    background: hsl(var(--chart-4) / 0.4);
  }
  ...
  .hpt-16 {
    ...
    background: hsl(var(--chart-5) / 0.6);
  }
...
══════════════════════════════════════════════════════ */
  ...
  .role-nav-btn.active .role-nav-icon {
    color: hsl(var(--foreground));
  }
...
══════════════════════════════════════════════════════ */
  ...
  .badge-pop-leave-active {
    animation: badge-out 0.25s ease-in;
  }
  ...
  .slide-up-delay-enter-active {
    animation: slide-in 0.55s 0.08s cubic-bezier(0.22, 1, 0.36, 1) both;
  }
  ...
  .slide-up-delay2-enter-active {
    animation: slide-in 0.6s 0.15s cubic-bezier(0.22, 1, 0.36, 1) both;
  }

  .slide-up-delay2-leave-active {
    animation: slide-out 0.25s ease-in;
  }

  .slide-up-delay3-enter-active {
    animation: slide-in 0.65s 0.22s cubic-bezier(0.22, 1, 0.36, 1) both;
  }

  .slide-up-delay3-leave-active {
    animation: slide-out 0.25s ease-in;
  }
