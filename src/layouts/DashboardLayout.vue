<template>
  <div class="bg-background text-foreground antialiased flex overflow-hidden h-full w-full">
    <!-- SIDEBAR UNIQUE -->
    <UnifiedSidebar :expanded="sidebarExpanded" :is-open="mobileMenuOpen"
      @toggle-expand="sidebarExpanded = !sidebarExpanded" @close-mobile="mobileMenuOpen = false" />

    <!-- OVERLAY MOBILE -->
    <div v-if="mobileMenuOpen" @click="mobileMenuOpen = false"
      class="fixed inset-0 bg-black/50 z-40 backdrop-blur-sm lg:hidden transition-opacity duration-300"></div>
    ...
    <div class="flex-1 flex flex-col h-full overflow-hidden">
      <!-- CONTENU DE LA PAGE -->
      <main class="flex-1 overflow-y-auto p-4 pt-4 lg:p-10  scroll-smooth">
        <slot />
      </main>
    </div>
  </div>
